...
final class LocationTVCell: UITableViewCell {
    ...
    private let typeLabel : UILabel = {
        let label = UILabel()
        label.translatesAutoresizingMaskIntoConstraints = false
        label.textColor = UIColor(named: "BlackColor")?.withAlphaComponent(0.7)
    ...
    private let dimensionLabel : UILabel = {
        let label = UILabel()
        label.translatesAutoresizingMaskIntoConstraints = false
        label.textColor = UIColor(named: "BlackColor")?.withAlphaComponent(0.4)
        label.font = .systemFont(ofSize: 18, weight: .regular)
        return label
    }()
...
//        contentView.backgroundColor = UIColor(named: "DarkGrey")
    ...
    public func configure(with viewModel: LocationTVCellVM) {
